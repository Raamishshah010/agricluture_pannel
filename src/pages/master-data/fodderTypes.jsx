...
import service from '../../services/fodderService';
...
export default function FodderTypes() {
    ...
    const handleSubmit = async () => {
        if (!formData.name) {
            alert(t('fodderTypes.validation.requiredFields'));
            return;
        }
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('fodderTypes.deleteConfirmation'))) {
            try {
       ...
       <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('fodderTypes.title')}</h1>
                    </div>
                    <button
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('fodderTypes.nameArabic')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('fodderTypes.nameEnglish')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('fodderTypes.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('fodderTypes.actions')}</th>
                                </tr>
                            </thead>
                    ...
                    {items.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                            {t('fodderTypes.emptyState')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('fodderTypes.totalItems')}: {items.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingEmirate ? t('fodderTypes.editItem') : t('fodderTypes.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={t('fodderTypes.nameArabicPlaceholder')}
                                    />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('fodderTypes.nameEnglishPlaceholder')}
                                    />
                                </div>
                            </div>
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingEmirate ? t('fodderTypes.update') : t('fodderTypes.add')}
                                        </>)
                                    }
