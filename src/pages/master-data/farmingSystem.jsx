...
import service from '../../services/farmingSystemService';
...
export default function FarmingSystem() {
    ...
    const handleSubmit = async () => {
        if (!formData.name) {
            toast.error(t('farmingSystem.fillRequiredFields'));
            return;
        }
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('farmingSystem.deleteConfirm'))) {
            try {
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('farmingSystem.nameEnglish')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('farmingSystem.nameArabic')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('farmingSystem.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('farmingSystem.actions')}</th>
                                </tr>
                            </thead>
                ...
                <div className="mt-4 text-sm text-gray-600">
                    {t('farmingSystem.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('farmingSystem.editItem') : t('farmingSystem.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('farmingSystem.nameEnglishLabel')} *
                                    </label>
                                    <input
                                        type="text"
                                        name="name"
                                        ...
                                        placeholder={t('farmingSystem.nameEnglishPlaceholder')}
                                    />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('farmingSystem.nameArabicLabel')} *
                                    </label>
                                    <input
                                        type="text"
                                        name="nameInArrabic"
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('farmingSystem.nameArabicPlaceholder')}
                                    />
                                </div>
                            </div>
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingItem ? t('farmingSystem.update') : t('farmingSystem.add')}
                                        </>)
                                    }
