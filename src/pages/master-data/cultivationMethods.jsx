...
import service from '../../services/cultivationMethodsService';
...
export default function CultivationMethods() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('cultivationMethods.deleteConfirm'))) {
            try {
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('cultivationMethods.title')}</h1>
                    </div>
                    <button
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('cultivationMethods.name')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('cultivationMethods.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('cultivationMethods.actions')}</th>
                                </tr>
                            </thead>
                    ...
                    {list.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                            {t('cultivationMethods.noItemsFound')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('cultivationMethods.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('cultivationMethods.editItem') : t('cultivationMethods.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('cultivationMethods.nameLabel')} *
                                    </label>
                                    <input
                                        type="text"
                                        name="name"
                                        ...
                                        placeholder={t('cultivationMethods.enterName')}
                                    />
                                </div>
                            </div>
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingItem ? t('cultivationMethods.update') : t('cultivationMethods.add')}
                                        </>)
                                    }
