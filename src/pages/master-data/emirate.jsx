...
export default function Emirates() {
    ...
    const openEditModal = (emirate) => {
        ...
        setFormData({
            name: emirate.name,
            nameInArrabic: emirate.nameInArrabic,
        });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (editingEmirate) {
                ...
                setEmirates(prev => prev.map(emirate =>
                    emirate.id === editingEmirate.id
                        ? { ...emirate, ...formData, updatedAt: new Date().toISOString() }
                        : emirate
                ));
            } else {
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('emirates.deleteConfirm'))) {
            try {
                await emirateService.deleteEmirate(id);
                setEmirates(prev => prev.filter(emirate => emirate.id !== id));
            } catch (error) {
                setLoading(false);
                toast.error(error.response?.data?.message || error.message);
            }
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('emirates.title')}</h1>
                    </div>
                    <button
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('emirates.nameArabic')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('emirates.nameEnglish')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('emirates.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('emirates.actions')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200">
                                {emirates.map((emirate) => (
                                    <tr key={emirate.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 text-sm font-medium text-gray-900">{emirate.nameInArrabic}</td>
                                        <td className="px-6 py-4 text-sm font-medium text-gray-900">{emirate.name}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{formatDate(emirate.createdAt)}</td>
                    ...
                    {emirates.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                            {t('emirates.noItemsFound')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('emirates.totalItems')}: {emirates.length}
                </div>
            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={t('emirates.nameArabicPlaceholder')}
                                    />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('emirates.nameEnglishPlaceholder')}
                                    />
                                </div>
                            </div>
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingEmirate ? t('emirates.update') : t('emirates.add')} {t('emirates.item')}
                                        </>)
                                    }
