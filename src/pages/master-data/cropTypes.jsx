...
import service from '../../services/cropTypeService';
...
export default function CropTypes() {
    ...
    const handleSubmit = async () => {
        if (!formData.name) {
            toast.error(t('cropTypes.fillRequiredFields'));
            return;
        }
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('cropTypes.deleteConfirm'))) {
            try {
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('cropTypes.title')}</h1>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <MasterDataCsvToolbar
                            items={list}
                            exportFields={['name', 'productionValue']}
                            exportFileName="crop-types.csv"
                            importLabel={t('common.importCsv') || 'Import CSV'}
                            exportLabel={t('common.exportCsv') || 'Export CSV'}
                            itemLabel="crop types"
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    {loading ? (
                        ...
                        <div className="text-center py-12 text-gray-500">
                            {t('cropTypes.noItemsFound')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('cropTypes.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('cropTypes.editItem') : t('cropTypes.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingItem ? t('cropTypes.update') : t('cropTypes.add')}
                                        </>)
                                    }
