...
export default function Regions() {
    ...
    const handleSubmit = async () => {
        if (!formData.name || !formData.nameInArrabic) {
            toast.error(t('regions.validation.allFieldsRequired'));
            return;
        }
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('regions.confirm.deleteItem'))) {
            try {
       ...
       <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('regions.title')}</h1>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <MasterDataCsvToolbar
                            items={list}
                            exportFields={['nameInArrabic', 'name']}
                            exportFileName="regions.csv"
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    {loading ? (
                        ...
                        <div className="overflow-x-auto">
                            ...
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('regions.nameArabic')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('regions.nameEnglish')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('regions.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('regions.actions')}</th>
                                </tr>
                            </thead>
                    ...
                    {!loading && list.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                            {t('regions.empty.noItems')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('regions.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('regions.editItem') : t('regions.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder={t('regions.nameArabicPlaceholder')}
                                    />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('regions.nameEnglishPlaceholder')}
                                    />
                                </div>
                            </div>
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingItem ? t('regions.update') : t('regions.add')}
                                        </>)
                                    }
