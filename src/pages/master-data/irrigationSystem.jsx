...
import service from '../../services/irrigationSystem';
...
export default function IrrigationSystem() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('irrigationSystem.confirmDelete'))) {
            try {
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 flex flex-col-reverse md:flex-row mt-2 justify-between items-center">
                    <div>
                        <h1 className="text-xl md:text-3xl font-bold text-gray-900">{t('irrigationSystem.title')}</h1>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <MasterDataCsvToolbar
                            items={list}
                            exportFields={['name', 'nameInArrabic']}
                            exportFileName="irrigation-system.csv"
                            importLabel={t('common.importCsv') || 'Import CSV'}
                            exportLabel={t('common.exportCsv') || 'Export CSV'}
                            itemLabel="irrigation systems"
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="bg-gray-100 border-b border-gray-200">
                                    <tr>
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('irrigationSystem.nameEnglish')}</th>
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('irrigationSystem.nameArabic')}</th>
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('irrigationSystem.createdAt')}</th>
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('irrigationSystem.actions')}</th>
                                    </tr>
                                </thead>
                    ...
                    {!loading && list.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                            {t('irrigationSystem.noItemsFound')}
                        </div>
                    )}
                </div>

                <div className="mt-4 text-sm text-gray-600">
                    {t('irrigationSystem.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('irrigationSystem.editItem') : t('irrigationSystem.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                        ...
                        <div className="px-6 py-4">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('irrigationSystem.nameEnglish')} *
                                    </label>
                                    <input
                                        type="text"
                                        name="name"
                                        ...
                                        placeholder={t('irrigationSystem.enterName')}
                                    />
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('irrigationSystem.enterNameArabic')}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                                ...
                                >
                                    {editingItem ? t('irrigationSystem.update') : t('irrigationSystem.add')}
                                </button>
                            </div>
                        </div>
                    </div>
