...
import service from '../../services/externalIrrigationSystem';
...
export default function ExternalIrrigationSystem() {
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            ...
            nameInArrabic: formData.nameInArrabic ? formData.nameInArrabic.trim() : ''
        };

        try {
            setLoading(true);
            if (editingItem) {
                await service.updateItem(editingItem.id, payload);
    ...
    const handleDelete = async (id) => {
        if (window.confirm(t('externalIrrigationSystem.confirmDelete'))) {
            try {
        ...
        <div className="min-h-screen bg-gray-50 p-0 md:p-8">
            <div className="max-w-7xl mx-auto">
                    ...
                    {loading ? (
                        ...
                        <div className="overflow-x-auto">
                            ...
                            <thead className="bg-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('externalIrrigationSystem.name')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('externalIrrigationSystem.nameArabic')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('externalIrrigationSystem.createdAt')}</th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{t('externalIrrigationSystem.actions')}</th>
                                </tr>
                            </thead>
                ...
                <div className="mt-4 text-sm text-gray-600">
                    {t('externalIrrigationSystem.totalItems')}: {list.length}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-900">
                                {editingItem ? t('externalIrrigationSystem.editItem') : t('externalIrrigationSystem.addNewItem')}
                            </h2>
                            <button
                                onClick={closeModal}
                        ...
                        <div className="px-6 py-4">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('externalIrrigationSystem.nameArabicLabel')}
                                    </label>
                                    <input
                                        type="text"
                                        name="nameInArrabic"
                                        ...
                                        placeholder={t('externalIrrigationSystem.nameArabicPlaceholder')}
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {t('externalIrrigationSystem.name')} *
                                    </label>
                                    <input
                                        type="text"
                                        name="name"
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                                        placeholder={t('externalIrrigationSystem.enterName')}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                                ...
                                >
                                    {t('externalIrrigationSystem.cancel')}
                                </button>
                                <button
                                ...
                                >
                                    {
                                        loading ? (
                                            ...
                                            {editingItem ? t('externalIrrigationSystem.update') : t('externalIrrigationSystem.add')}
                                        </>)
                                    }
