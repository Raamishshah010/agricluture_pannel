...
export default function Index(props) {
    ...
    const [activeTab, setActiveTab] = useState(() => sessionStorage.getItem('farmerActiveTab') || 'farmers');
    ...
    useEffect(() => {
        if (selectedFarm) {
            sessionStorage.setItem('farmerSelectedFarm', JSON.stringify(selectedFarm));
        } else {
    ...
    useEffect(() => {
        service.getFarmers(1, 50).then(res => {
            setList(res.data);
    ...
    const loadMore = async (currentPage) => {
        setLoading(true);
        service.getFarmers(currentPage, 50).then(res => {
            setList(res.data);
            setCount(res.pagination.totalPages);
            setPage(currentPage);
            setLoading(false);
    ...
    const handleFarms = async (item) => {
        ...
        service.getFarmerFarms(item.id).then(res => {
            setActiveTab("farms");
            setSelectedFarmer(item);
            setFarms(res.data)
        })
    ...
    const downloadPDF = async () => {
        ...
        const hasArabicContent = allFarmers.some(farmer => 
            hasArabic(farmer.name)
        );
        
        doc.setFontSize(18);
        doc.text('Farmers List', 14, 22);
        
        // Add filter info if search query exists
        if (query.trim()) {
            doc.setFontSize(10);
            doc.text(`Filter: ${query}`, 14, 30);
        }
        
        // Add total count
        doc.setFontSize(10);
        doc.text(`Total Records: ${allFarmers.length}`, 14, query.trim() ? 36 : 30);
        
        const tableData = allFarmers.map(farmer => [
        ...
        autoTable(doc, {
            ...
            body: tableData,
            startY: query.trim() ? 42 : 36,
            styles: { 
                fontSize: 9,
                font: hasArabicContent ? 'helvetica' : 'helvetica',
                halign: 'left'
            },
            headStyles: { 
                fillColor: [34, 197, 94],
        ...
        doc.save(`farmers-list-all-pages${query ? '-filtered' : ''}.pdf`);
    ...
    const downloadExcel = async () => {
        ...
        const worksheet = XLSX.utils.json_to_sheet(
            allFarmers.map(farmer => ({
                [t('farmers.image')]: farmer.image ? t('farmers.imageAvailable') : t('farmers.noImage'),
        ...
        XLSX.writeFile(workbook, `farmers-list-all-pages${query ? '-filtered' : ''}.xlsx`);
        setIsDownloadOpen(false);
        toast.success(t('farmers.downloadSuccess'));
    };
    ...
    return activeTab.includes("farmers") ? (
        <div className="max-w-5xl mx-auto">
            <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">{t('farmers.title')}</h1>
            ...
            {loading ? (
                <div className='flex justify-center items-center mt-60'>
                    <svg aria-hidden="true" className="w-8 h-8 text-green-600 animate-spin dark:text-gray-200 fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <>
                    ...
                    />
                    <Pagination initialPage={page} totalPages={count} onPageChange={loadMore} />
                </>
            )}
        </div>
