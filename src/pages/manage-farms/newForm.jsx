...
export const NewFarmForm = React.memo(({ onSave, onCancel }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        phoneNumber: '',
        region: regions[0]?.id || '',
        primaryCrop: "b1266a9f-d600-484c-93bf-3dafb7de141b",
        primaryVariety: "77a1fe4b-f4fb-48fe-a503-6e2dafdd8ebc",
        emiratesID: '',
        notes: '',
        owner: '',
        holder: '',
        ...
        farmingSystem: [],
        possessionStyle: '',
        numberOfDestinationMachines: 1,
        landUse: {
            arrableLand: {
                vegetablesOpen: 1,
                fruitPalmTreesOpen: 1,
                fieldCropsFodder: 0,
                leftForRest: 0,
                ornamentalTrees: 0,
                nurseries: 0
    ...
    function validate(obj, parentKey = "") {
        const msgObj = errorMessages;
        for (const key in obj) {
            if (key === "crops") continue;
            const fullKey = parentKey ? `${parentKey}.${key}` : key;
            const value = obj[key];
            const messageForKey = msgObj?.[key];
            if (value === "") {
                return messageForKey || `${fullKey} is required`;
            }
            if (typeof value === "number" && value === 0) {
                return messageForKey || `${fullKey} cannot be 0`;
            }
            if (Array.isArray(value) && value.length === 0) {
                return messageForKey || `${fullKey} cannot be empty`;
            }
            if (typeof value === "object" && value !== null) {
                if (fullKey === "coordinates") {
                    console.log(fullKey, obj,value);
                    
                    if (value.lat === 0 || value.lng === 0) {
                                            console.log(fullKey, obj,value);
                        return msgObj["coordinates"] || "Coordinates are required";
                    }
                } else {
                    const nestedMsgObj = msgObj?.[key] || {};
                    const nested = validate(value, nestedMsgObj, fullKey);
    ...
    const handleSubmit = (e) => {
        ...
        onSave({ ...formData, size: formData.totalArea });
    ...
    const previewCoords = useMemo(() => {
        const mapData = formData.mapData;
        const fruits = formData.crops.fruits.map(it => it.coordinates);
        const vegetables = formData.crops.vegetables.map(it => it.coordinates);
        const fodders = formData.crops.fieldCropsFodder.map(it => it.coordinates);
        const greenhouses = formData.crops.greenhouses.map(it => it.coordinates);
    ...
    return (
        <div className="min-h-0 h-full bg-gradient-to-br from-green-50 via-blue-50 to-emerald-50 p-0 md:p-6">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center gap-4 bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl shadow-2xl p-8 mb-6 text-white">
                    <button
                        onClick={onCancel}
                    ...
                    <h1 className="text-4xl font-bold">{t('farms.addNewFarm')}</h1>
                </div>
                <div>
                    <div className="bg-white rounded-xl shadow-lg p-6 mb-6 border border-gray-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="mb-4">
                                ...
                                <Dropdown
                                    options={farmers}
                                    value={farmers.find(it => it.id === formData.owner)}
                            ...
                            <div className="mb-4">
                                ...
                                <Dropdown
                                    options={farmers}
                                    value={farmers.find(it => it.id === formData.holder)}
                                    onChange={(item) => setFormData(pre => ({ ...pre, holder: item.id }))}
                                    placeholder={t('common.select')}
                                    classes='w-full'
                                />
                            </div>
                            ...
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, farmSerial: parseInt(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, farmNo: parseInt(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                            <div className="mb-4">
                            ...
                            <div className="mb-4">
                                <label className="block text-sm font-semibold text-gray-700 mb-1">
                                    {t('farms.region')} <span className="text-red-500">*</span>
                                </label>
                                <select
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, totalArea: parseFloat(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, noOfWorkers: parseInt(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, numberOfProductionWells: parseInt(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(pre => ({ ...pre, desalinationUnits: parseInt(e.target.value) }))}
                                    className="px-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                />
                            </div>
                        </div>
                            ...
                            <div key={index} className="bg-green-50 rounded-lg p-4 mb-4 border border-green-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div className='w-full'>
                                        ...
                                        <input
                                            type="number"
                                            placeholder={t('farms.numberOfAnimals')}
                                ...
                                <div className="bg-blue-50 rounded-lg p-4 mb-3 border-l-4 border-blue-400">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                        <div className='w-full'>
                                            ...
                                            <input
                                                type="number"
                                                placeholder={t('farms.numberOfGreenhouses')}
